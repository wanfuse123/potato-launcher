...
mdot = 3575.72
max_fuel = 45369
segment_length = 531.2888888888889
end_time = 400

# Define the function to calculate the rocket's trajectory
def rocket_trajectory(mass, velocity, position, specific_impulse, delta_t):
    ...
    def engine_force(m, time):
        # Calculate the shutter position
        shutter_position = total_barrel_length - (total_barrel_length - rocket_length) * time / (end_time / len(times))
    ...
    # Initialize arrays to store the rocket's motion data
    ...
    accelerations[0] = (gravity_force(mass) + engine_force(mass)) / mass
    masses[0] = mass
    ...
    for i in range(1, len(times)):
        ...
        mass = masses[i-1]
        fuel = fuels[i-1]
        acceleration = (gravity_force(mass) + engine_force(mass)) / mass
        ...
        forces[i] = gravity_force(mass) + engine_force(mass)
    ...
    return times, positions, velocities, accelerations, masses, fuels, forces

# Calculate the rocket's trajectory
delta_t = 0.1
position = np.array([0, altitude])
times, positions, velocities, accelerations, masses, fuels, forces = rocket_trajectory(mass, velocity, position, specific_impulse, delta_t)
